...
Soft X/Y alignment is normalized as a coordinate requirement. When an already-acquired Line-direction channel moves beyond its independent release radius while an acquired alignment still has a valid forward intersection with that direction, the direction remains in the acquired channel set without taking positional ownership; the alignment may therefore remain `snap.type` while the final endpoint satisfies both requirements. Incompatible requirements fail closed. Accepted directional semantics are grouped separately for transient representation: when Parallel and Perpendicular both truthfully describe the same authored Line direction, Parallel alone is the preferred transient description. Both accepted semantic IDs remain available to the click path, and current automatic commit policy persists both constraints in the same Line transaction.

Endpoint and Midpoint positions remain exact, finite-Line support remains bounded, H/V remains exclusive, Ctrl remains raw, and incompatible or degenerate semantic demands fail closed. Production-path regressions cover compatible and incompatible hard Endpoint semantics, continuous incident-Line continuation, a 90°/90° chain, compatible multi-direction coexistence, and the case where Point Reference owns position while Parallel alone is acquired; presentation, click, and one-transaction commit consume the accepted result. Automated tests are not browser acceptance: this behavior is **not [ACCEPTED / LOCKED]** and remains pending user browser verification.
