...
### 7.4 Geometric-demand equivalence and automatic persistence

**[IMPLEMENTED][IN PROGRESS] DETECTION IS NOT PERSISTENCE. COMPATIBILITY BEFORE PRIORITY. EQUIVALENCE BEFORE REPRESENTATION.** Line direction detections normalize to tolerance-compared, unoriented geometric demands while retaining their separate semantic family and reference identity. Equivalent demands are grouped before compatible distinct direction/coordinate requirements are composed into one accepted final endpoint. Exact Endpoint/Midpoint topology and finite-Line support remain hard positional authorities; `snap.type` describes position ownership rather than a semantic allow-list. Generic angular and point-reference construction can support the same geometry without thereby becoming persistent Line-to-Line constraints.

Transient representation and automatic persistence are separate projections of accepted semantic truth. Each equivalent demand group selects one deterministic preferred semantic representative. For an equivalent accepted Parallel + Perpendicular direction, both remain detected and usable geometrically, but Parallel alone is previewed and persisted; the redundant Perpendicular is rejected from that click transaction. Perpendicular alone remains a normal transient and automatically persistent constraint. This is transaction-scoped automatic authoring selection, not migration or global simplification of existing sketches.
...
Soft X/Y alignment is normalized as a coordinate requirement. When a Line-direction group in the existing release band has a valid forward intersection with an acquired alignment, the direction remains in the acquired channel set without taking positional ownership; the alignment may therefore remain `snap.type` while the final endpoint satisfies both requirements, including on a fresh chained segment. Incompatible requirements fail closed. Accepted directional semantics are grouped separately for transient representation and persistence: when Parallel and Perpendicular both truthfully describe the same authored Line direction, Parallel alone is the preferred transient and persistent description. Both accepted semantic IDs remain observable through semantic selection, while only the chosen minimal representative reaches automatic constraint creation.
...
1. **First: browser-verify equivalent Line-demand acquisition and minimal persistence.** The first 90° turn must preview and persist Perpendicular. At the second turn, equivalent Parallel + Perpendicular may both be detected and contribute to one final direction, while Parallel alone is previewed and persisted. Confirm final-point acquisition, threshold movement, no duplicate new Perpendicular marker, and hover-to-click geometry identity.
