...
- **2D Drawing** provides an SVG CAD-style canvas with Dimension and constraint workflows, shared `SketchPoint` topology, click and directional Window/Crossing multi-selection for Lines and Circles, snapping and geometric inference, Direct Manipulation, and Drawing Undo/Redo. `Line` authors one independent straight segment from P1 to P2 and completes; `Profile` authors continuing/chained connected straight segments. Both workflows store each segment as the same ordinary Line entity. The browser-accepted Circle Stage 1 authors true semantic Circle geometry from a persistent center point and scalar radius, presents the center and committed curve through shared Drawing roles, and reuses global point and Point-on-Curve snap semantics. Authoring Ctrl temporarily bypasses automatic acquisition/inference. Arc, Radius/Diameter Dimension and Constraint, Concentricity, and Tangency are not implemented.
