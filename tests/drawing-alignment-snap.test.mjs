...
assert.match(styles, /--drawing-alignment-guide:\s*#c084fc;[\s\S]*\.drawing-alignment-guide\s*\{[^}]*stroke:\s*var\(--drawing-alignment-guide\);[^}]*stroke-dasharray:\s*5 4;[^}]*pointer-events:\s*none;/s, 'separate purple guides are dashed and pointer-safe');
assert.match(styles, /\.drawing-line-preview\s*\{[^}]*stroke:\s*var\(--drawing-preview\);/s, 'alignment does not recolor the independent preview');
