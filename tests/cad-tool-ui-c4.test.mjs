...
const second = activation.resolveCadToolPointerActivation('profile', 1350, { x: 26, y: 20 }, first.record);
...
assert.deepEqual(lifecycle.finishDrawingConstruction(lifecycle.activateDrawingTool('profile', second.activationMode)), { activeTool: 'profile', activationMode: 'persistent' });
...
assert.match(workspace, /onDoubleClick=\{\(\) => \{ if \(activeTool === 'profile'\) finishProfile\(\)/, 'canvas dblclick finish remains present');
