...
assert.equal(DIMENSION_EDITOR_TEXT_SIZE_PX, 12);
assert.ok(DIMENSION_EDITOR_TEXT_SIZE_PX > DIMENSION_TEXT_SIZE_PX);
assert.equal(DIMENSION_EDITOR_HEIGHT_PX, 20);
...
assert.equal(dimensionEditorWidthPixels('95.623'), 52);
assert.equal(dimensionEditorWidthPixels('120'), 31);
assert.equal(dimensionEditorWidthPixels('120.5'), 45);
assert.equal(dimensionEditorWidthPixels('120.125'), 59);
...
assert.match(css, /drawing-dimension-editor[^}]*font-size: 12px;[^}]*font-weight: 400;/);
assert.match(css, /drawing-dimension-editor[^}]*color: #137a3e;/);
assert.match(css, /drawing-dimension-editor[^}]*line-height: 1\.2;/);
assert.ok(DIMENSION_EDITOR_HEIGHT_PX >= DIMENSION_EDITOR_TEXT_SIZE_PX * 1.2 + 2 * DIMENSION_EDITOR_VERTICAL_PADDING_PX + 2 * DIMENSION_EDITOR_BORDER_PX - 0.5, 'editor height fits the text line, padding, border, and caret');
