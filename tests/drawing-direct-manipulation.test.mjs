...
candidate=solveDrawingDragCandidate(document,{kind:'point',pointId:'p1'},{x:5,y:5}); assert.ok(candidate,'aligned target is projected instead of rejected');
sketch=candidate.sketches[candidate.activeSketchId];
assert.ok(Math.abs(Math.hypot(sketch.points.p1.x,sketch.points.p1.y)-10)<1e-7);
assert.deepEqual(sketch.points.p0,{id:'p0',x:0,y:0},'non-dragged endpoint is an exact stay when possible');
...
candidate=solveDrawingDragCandidate(horizontal,{kind:'point',pointId:'p1'},{x:1,y:5}); assert.ok(candidate);
assert.ok(Math.abs(candidate.sketches[candidate.activeSketchId].points.p1.x-10)<1e-7);
...
candidate=solveDrawingDragCandidate(vertical,{kind:'point',pointId:'p1'},{x:4,y:1}); assert.ok(candidate);
assert.ok(Math.abs(candidate.sketches[candidate.activeSketchId].points.p1.y)<1e-7);
...
assert.match(workspace,/activeTool === 'select'[\s\S]*resolveDimensionCandidate[\s\S]*resolveDrawingPointerOwner[\s\S]*setSelectedGeometry\(route\.selection\)/,'Select click uses shared semantic arbitration and unified multi-selection');
