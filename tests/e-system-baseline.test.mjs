...
assert.equal(getManufacturingPolicy('GENERATED_OUTER').compensationStrategy.name, 'offset', 'generated contours explicitly select OffsetStrategy');
assert.equal(getManufacturingPolicy('GENERATED_SLOT').compensationStrategy.name, 'no-movement', 'generated slots select NoMovementStrategy');
assert.equal(getManufacturingPolicy('IMPORTED_OUTER').compensationStrategy.name, 'no-movement', 'imported contours select NoMovementStrategy');
const boundaryFinalContourList = [
  ...
  { id: 'boundary-inner', source: 'final-contour', finalSource: 's-slot', kind: 'INNER', geometryType: 'GENERATED_SLOT', pathD: 'M 2 2 L 4 2 L 4 4 L 2 4 Z' },
];
...
assert.equal(importedFinalGeometry.contours.find((contour) => contour.finalSource === 'original-panel' && contour.kind === 'OUTER')?.geometryType, 'IMPORTED_OUTER', 'imported outer contours are typed as imported outer Final Geometry');
assert.equal(importedFinalGeometry.contours.find((contour) => contour.finalSource === 'original-panel' && contour.kind === 'INNER')?.geometryType, 'IMPORTED_HOLE', 'imported hole contours are typed as imported hole Final Geometry');
assert.equal(JSON.stringify(importedFinalGeometry.contours.find((contour) => contour.finalSource === 'original-panel' && contour.kind === 'OUTER')?.manufacturing), JSON.stringify({ clearance: false, slotClearance: false }), 'imported outer contours keep deprecated kerf-only manufacturing compatibility metadata');
assert.equal(JSON.stringify(importedFinalGeometry.contours.find((contour) => contour.finalSource === 'original-panel' && contour.kind === 'INNER')?.manufacturing), JSON.stringify({ clearance: false, slotClearance: false }), 'imported hole contours keep deprecated kerf-only manufacturing compatibility metadata');
