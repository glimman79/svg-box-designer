...
assert.ok(rail.indexOf('>Select</button>') < rail.indexOf('>Profile</button>'), 'Drawing tool order is Select then Line');
...
assert.match(rail, /cad-tool-button\$\{activeTool === 'profile' \? ' is-active' : ''\}/, 'Line active class remains derived from activeTool');
