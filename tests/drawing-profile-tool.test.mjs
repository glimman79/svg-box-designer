...
let document = types.createDrawingDocumentV2();
...
let interaction = lineTool.EMPTY_PROFILE_INTERACTION;
let result = lineTool.applyProfileClick(interaction, { x: 2.25, y: 3.75 }, createId);
...
interaction = lineTool.updateProfilePreview(interaction, { x: 7.125, y: 8.625 });
...
result = lineTool.applyProfileClick(interaction, { x: 7.125, y: 8.625 }, createId);
...
result = lineTool.applyProfileClick(interaction, { x: 11.2, y: -4.4 }, createId);
document = lineTool.appendEntityToActiveSketch(document, result.entity);
interaction = result.interaction;
...
assert.deepEqual(active.points[active.entities['line-test-2'].startPointId], { id: active.entities['line-test-2'].startPointId, ...result.entity.start }, 'simple Profile workflow continues from the prior Line endpoint');
assert.ok(Object.values(active.entities).every((entity) => entity.type === 'line'), 'Profile commits persistent Line entities');

const committedDocument = document;
interaction = lineTool.cancelProfileInteraction();
assert.deepEqual(interaction, lineTool.EMPTY_PROFILE_INTERACTION, 'Escape/tool switch cancellation clears transient state');
assert.equal(document, committedDocument, 'cancelling cannot alter committed geometry');

interaction = lineTool.applyProfileClick(interaction, { x: 5, y: 5 }, createId).interaction;
result = lineTool.applyProfileClick(interaction, { x: 5, y: 5 }, createId);
...
console.log('D2.2 persistent CAD Profile tool tests passed');
