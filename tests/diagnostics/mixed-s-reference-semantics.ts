...
const buildCase = (name: string, replacement: Replacement, fingerWidth=9.5) => {
  ...
  if(replacement!=='none'){
    ...
    const sparePanel=f.model.panels.find(p=>p.id!==ref.panelId&&p.id!==f.ownerPanelId)!;
    ...
    if(replacement==='TB') added=buildGeneratedTBGeometryItems(f.model,{
      [ref.sourceEdgeId]:{edgeAssignment:{connectionId:id,edgeRole:'A'}},[spareEdge]:{edgeAssignment:{connectionId:id,edgeRole:'B'}},
    } as any,{[id]:{id,prefix:'TB',properties:{fingerWidthMm:fingerWidth,isFingerWidthManual:true}}} as any,thicknessFor(f.model));
    else added=buildGeneratedSGeometryItems(f.model,{
      [ref.sourceEdgeId]:{slotAssignments:[{connectionId:id,slotRole:'A'}]},[spareEdge]:{slotAssignments:[{connectionId:id,slotRole:'B'}]},
    } as any,{[id]:{id,prefix:'S',properties:{slotLengthMm:10.7,isSlotLengthManual:true,slotOffsetMm:.8}}} as any,thicknessFor(f.model));
  }
  ...
  return {f,slot,ref,target,selectedSlot,source,added,along,distances};
};
...
const D=buildCase('case-D','TB',13.25);
const E=buildCase('case-E','S');
...
console.log('CASE F | INVALID | REPLACEMENT_CONFLICT | generatedGeometry=0');
