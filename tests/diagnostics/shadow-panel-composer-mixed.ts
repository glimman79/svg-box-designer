...
const generate = (name: string, spec: Spec) => {
  ...
  Object.entries(spec.tb ?? {}).forEach(([raw,role])=>{const side=+raw,id=`${name}-tb-${side}`;tbAssignments[owner.panel.edgeIds[side]]={edgeAssignment:{connectionId:id,edgeRole:role}};tbConnections[id]={id,prefix:'W',properties:{materialThicknessMm:spec.tbDepth??2.4,fingerWidthMm:spec.fingerWidth??30,isFingerWidthManual:spec.fingerWidth!==undefined}};});
