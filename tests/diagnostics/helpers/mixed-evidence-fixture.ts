...
const rectangle = (id: string, x: number, y: number, width: number, height: number, winding: Winding, reverse: boolean) => {
...
export const makeMixedFixture = (spec: MixedSpec) => {
  ...
  const owner=rectangle(`${spec.name}-owner`,x,y,width,height,winding,false);
  const rectangles=[owner]; const tbAssignments:any={},sAssignments:any={},tbConnections:any={},sConnections:any={};
  spec.tbEdges.forEach((edge,index)=>{const id=`${spec.name}-TB-${index}`; const mate=rectangle(`${id}-mate`,x+220,y+index*130,width,height,winding,!!spec.reverseTB); rectangles.push(mate);
  ...
  spec.sEdges.forEach((edge,index)=>{const id=`${spec.name}-S-${index}`;
    const mate=rectangle(`${id}-mate`,x+500,y+index*130,width,height,winding,!!spec.reverseSB); rectangles.push(mate);
