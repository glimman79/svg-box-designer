...
for (const [fixture, items, cohort] of [['S-only', sItems, 'S_ONLY'], ['TB-only', tbItems, 'TB_ONLY']] as const) {
  ...
  const enabled=selectGeneratedGeometryAuthority(model,items,'single-tool');
  const rollback=selectGeneratedGeometryAuthority(model,items,'legacy');
  const decision=enabled.decisions.find(value=>value.panelId==='owner');
  assert(decision?.authority==='COMPOSED'&&decision.cohort===cohort,`${fixture} was not enabled`);
  assert(enabled.generatedGeometry.filter(item=>item.kind==='PANEL_PATH'&&item.behaviour.replacesPanelId==='owner').length===1,`${fixture} boundary is not unique`);
  assert(JSON.stringify(buildFinalGeometry(model,[...enabled.generatedGeometry]))===JSON.stringify(buildFinalGeometry(model,[...baseline.generatedGeometry])),`${fixture} authoritative FinalGeometry changed`);
  assert(JSON.stringify(baseline.generatedGeometry)===JSON.stringify(rollback.generatedGeometry),`${fixture} rollback changed legacy output`);
  assert(enabled.panelCompositionModel==='relationship-composed-single-tool-v1','composition marker missing');
}
...
const mixedAuthority=selectGeneratedGeometryAuthority(model,mixedItems,'single-tool');
assert(mixedAuthority.decisions.find(x=>x.panelId==='owner')?.reason==='MIXED_NOT_ENABLED','mixed panel became authoritative');
assert(JSON.stringify(mixedAuthority.generatedGeometry)===JSON.stringify(mixedItems),'mixed legacy array changed');
...
console.log('Generated geometry Phase 4 single-tool authority: PASS');
