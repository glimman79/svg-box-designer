...
assert.deepEqual(DEFAULT_PROJECT_SETTINGS, { kerfMm: 0.15, profileOffsetMm: 0, slotClearanceMm: -0.10, selectedProfileOffsetIds: [], tapClearanceMm: -0.10 }, 'Tap Clearance defaults to -0.10 while every existing manufacturing default remains unchanged');
...
assert.deepEqual(JSON.parse(JSON.stringify(legacySettings)), { kerfMm: 0.15, profileOffsetMm: 0.25, slotClearanceMm: -0.10, selectedProfileOffsetIds: ['profile-1'], tapClearanceMm: -0.10 }, 'older settings gain the default Tap Clearance value without persisting legacy tap selections');
const settingsWithLegacyTapSelection = normalizeProjectSettings(JSON.parse('{"tapClearanceMm":-0.2,"selectedTapIds":["generated-tap:legacy"]}')).settings;
assert.deepEqual(settingsWithLegacyTapSelection, { kerfMm: 0.15, profileOffsetMm: 0, slotClearanceMm: -0.10, selectedProfileOffsetIds: [], tapClearanceMm: -0.2 }, 'legacy tap selections are ignored during loading and omitted from normalized settings');
