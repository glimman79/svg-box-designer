...
test('chain workflow metadata cannot change ordinary perpendicular resolution', () => {
  const start = { x: 0, y: 0 }, raw = { x: 2, y: 30 };
  const perpendicular = { type: 'perpendicular', entityId: 'previous', effectivePoint: { x: 0, y: 30 },
  ...
  assert.equal(automaticAxisConstraintKind(chained.interaction), automaticAxisConstraintKind(manual.interaction),
    'any final H/V classification is ordinary geometry behavior, not chain-history behavior');
});
