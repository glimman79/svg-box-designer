...
import { appendDimension, canonicalDimensionReferencePairKey, createLinePairDimension, createLineToLineAngleDimension, displayedDimensionMeasurement, formatAngleDimension } from '../.test-build/drawing-line-angle/drawingDimension.js';
...
import { solveDrawingDimensionEdit, verifyDrawingConstraints, verifyDrawingDrivingDimensions } from '../.test-build/drawing-line-angle/drawingConstraintSolver.js';
...
assert.equal(zeroEdit.ok, true, 'zero degrees converts atomically to Parallel');
assert.equal(zeroEdit.document.sketches.s.dimensions['angle-1'], undefined, 'converted Angle Dimension is removed');
assert.deepEqual(zeroEdit.document.sketches.s.dimensionOrder, [], 'converted Angle is removed from presentation order');
assert.equal(Object.values(zeroEdit.document.sketches.s.geometricConstraints).length, 1);
assert.deepEqual(zeroEdit.document.sketches.s.geometricConstraints['parallel:a:b'], { id: 'parallel:a:b', kind: 'PARALLEL', references: [{ kind: 'entity', entityId: 'a' }, { kind: 'entity', entityId: 'b' }] }, 'Parallel stores the canonical semantic Line pair');
assert.notDeepEqual(zeroEdit.document.sketches.s.points.aa, edited.document.sketches.s.points.aa, 'first-created Line absorbs conversion motion');
assert.deepEqual(zeroEdit.document.sketches.s.points.ba, edited.document.sketches.s.points.ba, 'later-created Line remains stable during conversion');
assert.deepEqual(zeroEdit.document.sketches.s.points.bb, edited.document.sketches.s.points.bb, 'selection/reference order does not move the later-created Line');
const reverseZeroCreation = structuredClone(edited.document); reverseZeroCreation.sketches.s.entityOrder = ['b', 'a'];
const reverseZero = solveDrawingDimensionEdit({ document: reverseZeroCreation, dimensionId: 'angle-1', targetValue: 0 });
assert.equal(reverseZero.ok, true); assert.deepEqual(reverseZero.document.sketches.s.points.aa, reverseZeroCreation.sketches.s.points.aa); assert.deepEqual(reverseZero.document.sketches.s.points.ab, reverseZeroCreation.sketches.s.points.ab); assert.notDeepEqual(reverseZero.document.sketches.s.points.ba, reverseZeroCreation.sketches.s.points.ba, 'reversing creation order reverses conversion ownership');
assert.equal(analyzeDrawingConstraints(zeroEdit.document.sketches.s).components.find(({ geometricConstraintIds }) => geometricConstraintIds.includes('parallel:a:b')).constraintRank, 1, 'Parallel contributes one canonical rank equation');
const parallelTransaction = transactDrawingDocument(editTransaction.history, edited.document, () => zeroEdit.document);
assert.equal(parallelTransaction.history.undo.length, editTransaction.history.undo.length + 1, 'conversion is exactly one History step');
assert.deepEqual(undoDrawingDocument(parallelTransaction.history, parallelTransaction.document).document, edited.document, 'Undo restores original Angle and geometry');
const parallelDragged = solveDrawingDragCandidate(zeroEdit.document, { kind: 'point', pointId: 'aa' }, { x: 4, y: 3 });
assert.ok(parallelDragged, 'Parallel participates in component-solver Direct Manipulation');
const pa = parallelDragged.sketches.s.points, av = { x: pa.ab.x - pa.aa.x, y: pa.ab.y - pa.aa.y }, bv = { x: pa.bb.x - pa.ba.x, y: pa.bb.y - pa.ba.y };
assert.ok(Math.abs(av.x * bv.y - av.y * bv.x) / (Math.hypot(av.x, av.y) * Math.hypot(bv.x, bv.y)) < 1e-7, 'dragged Lines remain parallel');
const zs = zeroEdit.document.sketches.s, za = { ...zs.entities.a, start: zs.points.aa, end: zs.points.ab }, zb = { ...zs.entities.b, start: zs.points.ba, end: zs.points.bb };
const distance = createLinePairDimension(za, zb, { x: 0, y: 5 }, 'parallel-distance');
assert.equal(distance?.kind, 'LINE_TO_LINE_DISTANCE', 'existing Line pair tool recognizes solved Parallel geometry');
const withParallelDistance = appendDimension(zeroEdit.document, distance);
assert.equal(withParallelDistance.sketches.s.dimensions['parallel-distance'].role, 'driving', 'Parallel and separation are rank-independent');
const distanceEdited = solveDrawingDimensionEdit({ document: withParallelDistance, dimensionId: 'parallel-distance', targetValue: 50 });
assert.ok(distanceEdited.ok && verifyDrawingConstraints(distanceEdited.document.sketches.s, ['parallel-distance'], ['parallel:a:b']), 'Parallel and 50 mm distance coexist in the canonical solver');
const duplicateSource = structuredClone(edited.document); duplicateSource.sketches.s.geometricConstraints = structuredClone(zs.geometricConstraints); duplicateSource.sketches.s.geometricConstraintOrder = [...zs.geometricConstraintOrder];
const duplicateConversion = solveDrawingDimensionEdit({ document: duplicateSource, dimensionId: 'angle-1', targetValue: 0 });
assert.equal(duplicateConversion.ok, true); assert.equal(Object.keys(duplicateConversion.document.sketches.s.geometricConstraints).length, 1, 'existing canonical Parallel is reused, never duplicated');
