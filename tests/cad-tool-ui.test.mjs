...
assert.match(workspace, /className=\{`cad-tool-button\$\{activeTool === 'profile' \? ' is-active' : ''\}`\} aria-pressed=\{activeTool === 'profile'\}/, 'Line visual and accessible state derive from activeTool');
...
assert.deepEqual(lifecycle.finishDrawingConstruction(lifecycle.activateDrawingTool('profile', 'persistent')), { activeTool: 'profile', activationMode: 'persistent' }, 'persistent Line remains active between chains');
assert.doesNotMatch(workspace, /is-active[^\n]*(profileInteraction|unfinished|start)/, 'active styling is independent of unfinished Line state');
...
assert.match(workspace, /onPointerDown=\{\(event\) => activateToolFromPointer\('profile', event\)\}/, 'Line uses application-controlled pointer activation');
