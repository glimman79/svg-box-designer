...
assert.ok(rail.indexOf('>Select</button>') < rail.indexOf('>Profile</button>'), 'Select is the first Drawing tool and Line is second');
...
const first = activation.resolveCadToolPointerActivation('profile', 1000, { x: 20, y: 20 }, null);
assert.equal(first.activationMode, 'normal', 'first activation is normal immediately');
const closeSecond = activation.resolveCadToolPointerActivation('profile', 1300, { x: 24, y: 23 }, first.record);
...
assert.equal(activation.resolveCadToolPointerActivation('profile', 1351, { x: 20, y: 20 }, first.record).activationMode, 'normal', 'activation outside timing threshold stays normal');
assert.equal(activation.resolveCadToolPointerActivation('profile', 1200, { x: 27, y: 20 }, first.record).activationMode, 'normal', 'activation outside distance tolerance stays normal');
...
assert.deepEqual(lifecycle.activateDrawingTool('profile', first.activationMode), { activeTool: 'profile', activationMode: 'normal' });
assert.deepEqual(lifecycle.finishDrawingConstruction(lifecycle.activateDrawingTool('profile', closeSecond.activationMode)), { activeTool: 'profile', activationMode: 'persistent' }, 'persistent Line stays active after a chain finishes');
assert.match(rail, /<button type="button"[^>]*aria-pressed=\{activeTool === 'select'\}/, 'Select remains a native pressed-state button');
assert.match(rail, /<button type="button"[^>]*aria-pressed=\{activeTool === 'profile'\}/, 'Line remains a native pressed-state button');
