...
export type ArcToolInteraction = Readonly<{ start: AcceptedArcEndpoint | null; end: AcceptedArcEndpoint | null; form: DrawingPoint | null }>;
export const EMPTY_ARC_INTERACTION: ArcToolInteraction = { start: null, end: null, form: null };
...
export const updateArcPreview = (state: ArcToolInteraction, point: DrawingPoint): ArcToolInteraction =>
  state.start && state.end ? { ...state, form: point } : state;
...
export const acceptArcEndpoint = (state: ArcToolInteraction, endpoint: AcceptedArcEndpoint): ArcToolInteraction => {
  ...
  if (!state.start) return { start: endpoint, end: null, form: null };
  ...
  return { ...state, end: endpoint, form: null };
};
